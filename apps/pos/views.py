...
- Safe loyalty/membership/inventory/wash feature checks
"""
...
def feature_on(feature_name, default=True):
    """
    Safe feature flag check.
    If feature system/cache fails, returns default.
    """
    try:
        from apps.system.features import is_feature_enabled
        return is_feature_enabled(feature_name)
    except Exception as e:
        logger.warning(f'Feature flag check failed for {feature_name}: {e}')
        return default


def reserve_idempotency_key(cache_key):
    """
    Returns False if duplicate.
    If cache backend fails, allows request but logs warning.
    """
    try:
        ...
        logger.warning(f'POS idempotency cache unavailable: {e}')
        return True
...
def pos_home(request):
    ...
    wash_enabled = feature_on('wash')
    bookings_enabled = feature_on('bookings')
    inventory_enabled = feature_on('inventory')
    loyalty_enabled = feature_on('loyalty')
    memberships_enabled = feature_on('memberships')

    wash_job_id = request.GET.get('wash_job')
    booking_id = request.GET.get('booking')

    # ─── Prefill from Wash Job ───────────────────────────────
    if wash_job_id and wash_enabled:
        try:
            ...
            prefill = {
                ...
                'customer_id': job.customer.pk,
                'customer_name': job.customer.name,
                'vehicle_id': job.vehicle.pk,
                'vehicle_reg': job.vehicle.registration_number,
                'items': prefill_items,
            }
    ...
    elif booking_id and bookings_enabled:
        try:
            ...
            prefill = {
                ...
                'customer_id': booking.customer.pk,
                'customer_name': booking.customer.name,
                'vehicle_id': booking.vehicle.pk,
                ...
                'items': [{
                    ...
                    'vat': True,
                    'service_id': (
                        booking.service.pk if booking.service else None
                    ),
                    'product_id': None,
    ...
    context = {
        ...
        'tax_percentage': float(business.tax_percentage),
        'tax_name': business.tax_name,
        'prefill': prefill,
        'prefill_json': prefill_json,

        # Feature flags for templates if needed later
        'wash_enabled': wash_enabled,
    ...
    data = [
        {
            ...
            'meta': f'{p.sku} · Rs. {p.selling_price}',
        }
        for p in products
    ]
...
def wash_job_search_ajax(request):
    if not feature_on('wash'):
        return JsonResponse({'wash_jobs': []})
    ...
    jobs = WashJob.objects.select_related(
        ...
        status__in=[
            WashJob.STATUS_WAITING,
            WashJob.STATUS_ASSIGNED,
            WashJob.STATUS_WASHING,
            WashJob.STATUS_QUALITY_CHECK,
    ...
    data = [
        {
            'id': j.pk,
            'label': f'{j.job_number} — {j.customer.name}',
            'meta': (
                f'{j.vehicle.registration_number} · '
                f'{j.service.name if j.service else "No service"} · '
                f'{j.get_status_display()}'
            ),
            'customer_id': j.customer.pk,
            'customer_name': j.customer.name,
            'vehicle_id': j.vehicle.pk,
            'vehicle_reg': j.vehicle.registration_number,
            'service_id': j.service.pk if j.service else None,
            'service_name': j.service.name if j.service else '',
...
def booking_search_ajax(request):
    ...
    data = [
        {
            'id': b.pk,
            'label': f'{b.booking_number} — {b.customer.name}',
            'meta': (
                f'{b.vehicle.registration_number} · '
                f'{b.service.name} · {b.scheduled_date}'
            ),
            'customer_id': b.customer.pk,
            'customer_name': b.customer.name,
            'vehicle_id': b.vehicle.pk,
            'vehicle_reg': b.vehicle.registration_number,
            'service_id': b.service.pk,
...
def service_price_for_vehicle(request):
    ...
    price_amount = service.get_price_amount(
...
def product_quick_create(request):
    if not feature_on('inventory'):
        return JsonResponse({
            'success': False,
            'error': 'Inventory feature is disabled.',
        }, status=403)
    ...
    if existing:
        return JsonResponse({
            ...
            'item': {
                ...
                'name': existing.name,
                'sku': existing.sku,
                'price': str(existing.selling_price),
                'meta': f'{existing.sku} · Rs. {existing.selling_price}',
            },
            'message': f'"{name}" exists — selected.',
        })
...
def wash_job_items_ajax(request, pk):
    if not feature_on('wash'):
        return JsonResponse({
            'success': False,
            'error': 'Wash feature is disabled.',
        }, status=403)
...
def complete_sale(request):
    ...
    if not reserve_idempotency_key(cache_key):
        return JsonResponse({
            'success': False,
    ...
    try:
        with transaction.atomic():
                ...
                if vehicle.customer_id != customer.pk:
                    if wash_job_id and wash_enabled:
                        logger.info(
                            ...
                            f'reassigned from customer '
                            f'{vehicle.customer_id} to {customer.pk} '
                            f'via wash job {wash_job_id}'
                        )
                        vehicle.customer = customer
                        vehicle.save(update_fields=['customer', 'updated_at'])
                    else:
                        raise ValueError(
                            f'Vehicle {vehicle.registration_number} does not '
                            f'belong to customer {customer.name}.'
                        )
            ...
            if wash_job_id:
                if not wash_enabled:
                    raise ValueError('Wash feature is disabled.')

                wash_job = WashJob.objects.filter(
                    pk=wash_job_id
                ).first()
            ...
            if booking_id:
                if not bookings_enabled:
                    raise ValueError('Bookings feature is disabled.')

                booking = Booking.objects.filter(
                    pk=booking_id
                ).first()

            elif wash_job and wash_job.booking and bookings_enabled:
            ...
            for item in items:
                ...
                if item_type == 'product' and not inventory_enabled:
                    raise ValueError('Inventory feature is disabled.')
                ...
                line_tax = (
                    line_subtotal * tax_pct / Decimal('100')
                ...
                invoice_type = (
                    'service'
                    if item_type in ('service', 'repair', 'custom')
                    else 'product'
                )
            ...
            for payment_data in payments:
                try:
                    amt = Decimal(str(payment_data.get('amount', 0)))
                except (InvalidOperation, ValueError):
            ...
            if abs(payment_total - grand_total) > Decimal('0.01'):
                raise ValueError(
                    f'Payment (Rs. {payment_total}) does not match '
            ...
            for item_data in resolved_items:
                InvoiceItem.objects.create(
                    ...
                    description_snapshot=item_data['description'],
            ...
            for payment_data in payments:
                pm_code = payment_data.get('method_code', '').strip()
            ...
            if inventory_enabled:
                for item_data in resolved_items:
                    if (
                        ...
                        stock.quantity = before - item_data['quantity']
                        ...
                        InventoryTransaction.objects.create(
                            ...
                            unit_cost=item_data['product'].cost_price,
                            reference_id=str(order.pk),
                            reference_type='Order',
                            notes=f'Sale via {order.order_number}',
                            created_by=request.user,
                        )

            # ─── Wash Job Linking / Auto Completion ──────────
            if wash_enabled and wash_job:
                wash_job.order = order
                wash_job.invoice = invoice
                wash_job.status = WashJob.STATUS_COMPLETED
                wash_job.completed_at = timezone.now()
                wash_job.save(update_fields=[
                    'order',
                    'invoice',
                    'status',
                    'completed_at',
            ...
            elif wash_enabled and has_service and vehicle:
                try:
                    first_service = None
                    ...
                    for item_data in resolved_items:
                        if (
                            ...
                            and item_data['service']
                        ):
                            first_service = item_data['service']
                            break
            ...
            if loyalty_enabled:
                try:
                    ...
                    if config.is_enabled and config.points_per_currency > 0:
                        ...
                        if points > 0:
                            award_points(
                                customer=customer,
                                points=points,
                                tx_type=LoyaltyTransaction.TYPE_EARN,
                                description=(
                                    f'Earned from {order.order_number}'
                                ),
                                reference_id=str(order.pk),
                                reference_type='Order',
                                user=request.user,
                    ...
                    logger.warning(f'Loyalty execution failed: {e}')

            # ─── Membership Block ────────────────────────────
            if memberships_enabled and has_service and vehicle:
                try:
                    ...
                    logger.warning(f'Membership execution failed: {e}')
